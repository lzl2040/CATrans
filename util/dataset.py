...
def make_dataset(split=0, data_root=None, data_list=None, sub_list=None, filter_intersection=False, mode='train'):
    assert split in [0, 1, 2, 3]
    if not os.path.isfile(data_list):
        raise (RuntimeError("Image list file do not exist: " + data_list + "\n"))
    if mode == 'train':
        split_data_list = 'pascal_train_split{}'.format(split) + '.pth'
    else:
        split_data_list = 'pascal_val_split{}'.format(split) + '.pth'
