...
def main():
    ...
    lr_sched = torch.optim.lr_scheduler.LambdaLR(optimizer, (lambda n:
                                                             1.0 if n <= 10 else
                                                             0.1))
    if main_process():
        print("model info")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.fix_random_seed_val:
            print('set seed')
            # setup_seed(args.manual_seed + epoch + 1, args.seed_deterministic)
        ...
        if args.distributed:
            ...
            # ----------------------  TRAIN  ----------------------
        mIoU_train, mAcc_train, allAcc_train = train(train_loader, model, optimizer, epoch)

        if main_process() and args.viz:
            writer.add_scalar('mIoU_train', mIoU_train, epoch_log)

        # save model for <resuming>
        if ((epoch + 1) % args.save_freq == 0) and ((epoch + 1) > 0) and main_process():
            filename = args.snapshot_path + '/epoch_{}.pth'.format(epoch + 1)
...
def train(train_loader, model, optimizer, epoch):
    ...
    for i, (q_input, target, s_input, s_mask, subcls) in enumerate(train_loader):
        ...
        # accuracy = (intersection_meter.val) / ((target_meter.val) + 1e-10)
