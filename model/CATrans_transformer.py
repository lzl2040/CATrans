...
class RAT(nn.Module):
    def __init__(self,num_heads,Fms_out,Fs_out,Fq_out,out_dim,final_out_dim):
        ...
        self.FFN = FFN(input_channel=out_dim, d_in=out_dim, d_hid=2048, d_out=final_out_dim)
...
class AttentionBlock(nn.Module):
    def __init__(self,input_dim,output_dim,num_heads,LN,FFN,dropout=0.1):
        ...
        # self.fc_qkv = nn.Linear(in_features=input_dim, out_features=output_dim,bias=False)
        ...
        self.fc_qkv = nn.Linear(in_features=input_dim, out_features=output_dim)
        self.LN = LN
        self.FFN = FFN
...
class Decoder(nn.Module):
    ...
    def forward(self,feature1,feature2,feature3,feature4):
        ...
        # 有大于1是正常的
        # predict_label = torch.softmax(predict_label,dim=1)
        # print(predict_label[0])
